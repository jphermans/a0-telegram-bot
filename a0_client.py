...
class A0Client:
    ...
    async def get_memory_subdirs(self) -> Dict[str, Any]:
        """Get available memory subdirectories.
        
        Returns:
            Dict with 'success', 'subdirs' list, and optional 'error'
        """
        try:
            session = await self._get_session()
            ...
            async with session.post(
                f"{self.endpoint}/memory_dashboard",
                json={"action": "get_memory_subdirs"},
                headers=headers
            ) as response:
                if response.status == 200:
                    return await response.json()
                return {"success": False, "error": f"API error: {response.status}", "subdirs": ["default"]}
        except Exception as e:
            logger.error(f"Failed to get memory subdirs: {e}")
            return {"success": False, "error": str(e), "subdirs": ["default"]}
    ...
    async def search_memories(
        ...
        """Search memories in A0.
        
        Args:
            query: Search query (empty to list all)
            memory_subdir: Memory subdirectory to search
            area: Filter by area (e.g., 'main', 'user')
            limit: Maximum number of results
            threshold: Similarity threshold for search
        
        Returns:
            Dict with success status, memories list, and metadata
        """
        try:
            session = await self._get_session()
            headers = {"Content-Type": "application/json", "X-API-KEY": self.api_key}
            ...
            async with session.post(
                ...
                if response.status == 200:
                    return await response.json()
                return {"success": False, "error": f"API error: {response.status}", "memories": [], "total_count": 0}
        except Exception as e:
            logger.error(f"Failed to search memories: {e}")
            return {"success": False, "error": str(e), "memories": [], "total_count": 0}
    
    async def delete_memory(self, memory_id: str, memory_subdir: str = "default") -> Dict[str, Any]:
    ...
    async def bulk_delete_memories(self, memory_ids: List[str], memory_subdir: str = "default") -> Dict[str, Any]:
